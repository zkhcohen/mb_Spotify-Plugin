...
namespace SpotifyAPI.Web.Auth
{
  public class EmbedIOAuthServer : IAuthServer
  {
    ...
    public event Func<object, PkceResponse, Task>? PkceReceived;
    ...
    public EmbedIOAuthServer(Uri baseUri, int port, Assembly resourceAssembly, string resourcePath)
    {
      ...
      _webServer = new WebServer(port)
        .WithModule(new ActionModule("/callback", HttpVerbs.Post, (ctx) =>
        {
            ...
            PkceReceived?.Invoke(this, new PkceResponse(query["code"]!)
            {
                State = query["state"]
            });
